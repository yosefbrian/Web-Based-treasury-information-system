...
<ol class="breadcrumb">
  ...
  <li><a href="#">Peraturan Tunjangan Kinerja</a></li>
</ol>
...
<h2>PERATURAN TUNJANGAN KINERJA</h2>
<div class="x_title">
</div>

@if(Session::has('flash_message_tambah'))
    <div class="alert alert-success"><strong>Sukses!</strong> Anda berhasil menambah peraturan tunjangan kinerja.<em> {!! session('flash_message') !!}</em></div>
@endif
@if(Session::has('flash_message_hapus'))
    <div class="alert alert-success"><strong>Sukses!</strong> Anda berhasil menghapus peraturan tunjangan kinerja.<em> {!! session('flash_message') !!}</em></div>
@endif
            ...
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <input type="hidden" value="<?php echo $tukin->id;?>" name="id">
                        <h5>Apakah Anda yakin akan menghapus data ini?</h5>
                    </div>
                    <div class="modal-footer">
                        <a class="btn btn-info btn-simple pull-left" data-dismiss="modal" style="width:60px;">Tidak</a>
                        <a class="btn btn-danger btn-simple pull-right" title="Hapus" href="{{ action('PeraturanController@deletetukin', $tukin->id) }}" style="width:60px;">Ya</a>
                    </div>
                </div>
            </div>
        </div>
...
<form action="{{ url('peraturantukin') }}" method="post" enctype="multipart/form-data">
